...
import json
#-------------------rs--------------------------#
def rs_encode(data,rs_length):
    rs_obj = RSCodec(rs_length)
    return [rs_obj.encode(d) for d in data]

def rs_decode(data,rs_length):
    if rs_length == 0:
        return data,[]
    rs_obj = RSCodec(rs_length)
    re_data = []
    ...
    for i,d in enumerate(data):
        try:
            corrected = rs_obj.decode(data)
            right_data.append(corrected)
        except:
            logging.debug('can not correct ori data')
            re_data.append(d[:-rs_length])
            bad_pos.append(i)
    return re_data, bad_pos
#----------------parsers--------------------#
def file_name_pure(file_name):
    return file_name.split('/')[-1]

def file_name_no_type(file_name):
    return file_name_pure(file_name).split('.')[0]
...
#-----------------segmentation----------------#
def join_and_save(data,file_name):
...
def data_to_indexed_dnas(data,index_length = None):
    index_l = index_length
    if index_l == None:
        index_l = index_len(len(data))
    return data_to_dnas(data,index_l), index_l
...
def data_to_dnas(data,index_length = 8):
    ...
    for i,d in enumerate(data):
        ...
        dnas.append(d)
        # except:
            # print(i,num_to_dna(i,index_length),byte_to_dna(d))
    return dnas
...
def dnas_to_data(dnas,chunk_num,index_length = 8):
    ...
    for dna in dnas:
        ...
        try:
            ...
            print(index,chunk_num)
    return data_chunks

def index_len(chunk_num):
    return int(log(chunk_num,4) + 1)